...
namespace MagicalNuts
{
	...
	public class IncrementalTextBox : TextBox
	{
		...
		/// 候補リスト項目を表します。
		/// </summary>
		public class CandidateListViewItem : ListViewItem
		{
			/// <summary>
			/// 候補オブジェクト
			/// </summary>
			...
			public CandidateListViewItem(object candidate) : base()
			{
				Candidate = candidate;
		...
		/// 候補リスト項目の辞書
		/// </summary>
		public Dictionary<string, List<CandidateListViewItem>> _CandidateListViewItemDictionary = null;

		/// <summary>
		/// 候補リスト項目の辞書
		/// </summary>
		public Dictionary<string, List<CandidateListViewItem>> CandidateListViewItemDictionary
		{
			set
			{
		...
		/// <returns></returns>
		public static async Task<Dictionary<string, List<CandidateListViewItem>>> GetCandidateListViewItemDictionaryAsync(object[] candidates
		...
		/// <param name="mh">キーに対して候補が相応しいか判定するデリゲート</param>
		/// <returns></returns>
		public static Dictionary<string, List<CandidateListViewItem>>  GetCandidateListViewItemDictionary(object[] candidates
			, KeysForDictionaryHandler kfdh = null, MatchHandler mh = null)
		{
			// CandidateListViewItemに変換
			List<CandidateListViewItem> allitems = new List<CandidateListViewItem>();
			foreach (object candidate in candidates)
			{
				allitems.Add(new CandidateListViewItem(candidate));
			}

			// 候補リスト項目の辞書作成
			Dictionary<string, List<CandidateListViewItem>> dict = new Dictionary<string, List<CandidateListViewItem>>();
			foreach (CandidateListViewItem allitem in allitems)
			{
				// キーを取得
				string[] keys = null;
				...
				{
					// 既存を取得
					List<CandidateListViewItem> values = null;
					...
					{
						// 候補リスト抽出
						List<CandidateListViewItem> items = null;
		...
		private void DecideOnCandidateListView()
		{
			...
			// 選択項目取得
			CandidateListViewItem item = CandidateListView.SelectedItems[0] as CandidateListViewItem;
